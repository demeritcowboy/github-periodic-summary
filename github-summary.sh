...
{
  "query": "query { repository(owner:\"$GITHUB_REPO_OWNER\", name:\"$GITHUB_REPO_NAME\") { pullRequests(after:\"$GITHUB_SUMMARY_LASTCURSOR\", first:100, orderBy:{field:UPDATED_AT, direction:ASC}) { edges { node { number title updatedAt closed bodyText } cursor } } } }"
}
ENDQUERY
